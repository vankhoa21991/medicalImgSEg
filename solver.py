...
class Solver(object):
	...
	def test(self):
		...
		unet_path = os.path.join(self.model_path, 'U_Net-150-0.0002-57-0.0808.pkl')
		# U-Net Train
		if os.path.isfile(unet_path):
			# Load the pretrained Encoder
			self.unet.load_state_dict(torch.load(unet_path))
		...
		wr = csv.writer(f)
		wr.writerow(
			[self.model_type, acc, SE, SP, PC, F1, JS, DC, self.lr, 2, self.num_epochs, self.num_epochs_decay,
